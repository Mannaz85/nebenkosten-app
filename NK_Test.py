...
# Hilfsfunktion für Euro-Formatierung (Deutsch)
def fmt_eur(val):
...
def check_and_update_dates(df):
    ...
    for index, row in df.iterrows():
        if pd.notnull(row['Nächste Fälligkeit']):
            # Wir zwingen das Datum auf den reinen Tag-Wert
            current_due = row['Nächste Fälligkeit'].date()
            if current_due <= today:
                ...
                if turnus in INTERVALL_MONATE:
                    ...
                    # So lange erhöhen, bis es in der Zukunft liegt
    ...
    if updated:
        ...
        st.toast("🔄 Termine wurden aktualisiert!", icon="📅")
    return df
...
# TAB 1: DASHBOARD (ZUSAMMENFASSUNG & GRAFIK)
with tab1:
    if not df.empty:
        ...
        if not due_soon.empty:
            ...
            st.success("Keine dringenden Zahlungen in den nächsten 2 Wochen.")

        st.divider()
        
        # --- BERECHNUNGEN MONATLICHE KOSTEN ---
        ...
        # Jeweilige Last (50% Haus + 100% Privat)
        p1_total = (shared_total / 2) + p1_priv
        p2_total = (shared_total / 2) + p2_priv
        
        # Aktuelle Ansicht für den Nutzer
        ...
        compare_df = pd.DataFrame({
            "Person": PERSONEN, 
            "Monatliche Belastung": [p1_total, p2_total]
        })
        fig = px.bar(compare_df, x="Person", y="Monatliche Belastung", color="Person", text_auto='.2f',
                     color_discrete_map={PERSONEN[0]: '#636EFA', PERSONEN[1]: '#EF553B'})
        fig.update_layout(showlegend=False, height=350, yaxis_title="Euro pro Monat")
    ...
    if art_input:
        ...
        if matches:
            ...
            for i, m in enumerate(matches[:3]):
                if cols[i].button(m, key=f"m_{i}", use_container_width=True):
    ...
    with st.form("new_form", clear_on_submit=True):
        ...
        if st.form_submit_button("✅ Speichern", use_container_width=True):
            ...
            if betrag and final_art:
                ...
                st.success(f"'{final_art}' gespeichert!")
                st.rerun()
            else:
                st.error("Kategorie und Betrag fehlen!")

# TAB 3: LISTE (EDITOR)
with tab3:
    st.subheader("Vollständige Liste")
    if not df.empty:
        # Hier nutzen wir den data_editor für schnelles Bearbeiten/Löschen
